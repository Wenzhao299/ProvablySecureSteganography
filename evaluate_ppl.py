"""Compute perplexity statistics for generated stego text files."""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate PPL for stego text generated by stego_parallel.py"
    )
    parser.add_argument(
        "--model", type=str, default="qwen2.5", choices=SUPPORTED_MODEL_NAMES
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Path to stego file (.jsonl/.csv). Default: results_parallel/{dataset}/{method}.jsonl",
    )
    return parser.parse_args()


def safe_get_id_and_text(row: dict):
    """Safely get id and text."""
...
def main():
    ...
    results_file = args.results_file or os.path.join(
        "results_parallel", args.dataset, f"{args.method}.jsonl"
    )
    ...
    with open(output_file, "w", encoding="utf-8") as f_out:
        for context_id, generated_text in tqdm(
            iter_stego_samples(results_file), desc="Calculating PPL"
        ):
            try:
                encodings = tokenizer(
