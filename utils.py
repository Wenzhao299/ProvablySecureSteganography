"""Shared utilities for model loading, sampling, and steganography method dispatch."""
...
def random_bits(length: int) -> str:
    """Generate a random bit string of the requested length."""
...
def initialize_model_and_precision(args, seed: int):
    """Initialize model and precision."""
...
def encode_with_method(
    method: str, model, context_tokens: List[int], message_bits: str, args, device: str
) -> Tuple[List[int], str]:
    # Delayed imports avoid circular dependency with pss modules importing utils.
    """Encode message bits with the selected steganography method."""
    if method == "ac":
...
def decode_with_method(
    ...
    # Delayed imports avoid circular dependency with pss modules importing utils.
    """Decode hidden bits with the selected steganography method."""
    if method == "ac":
...
def _convert_token_to_id(self, token):
    """Internal helper to convert token to id."""
...
def limit_past(past):
    """Trim cached key/value tensors to the supported context window."""
...
def _past_seq_len(past):
    """Return sequence length stored in past key values."""
    if past is None:
        return 0
        ...
        while total_num < length:
            ...
            if past is None:
                ...
                try:
                    ...
                    msg = str(e)
                    if (
                        "past_key_values" in msg
                        and "Cache" in msg
                        and not isinstance(past, Cache)
                    ):
                        model_output = model(
                            prev_in,
...
def is_sent_finish(token_idx, enc):
    """Return whether a token ends a sentence."""
    token = enc.decode([token_idx])
    return "." in token or "!" in token or "?" in token


def num_same_from_beg(bits1, bits2):
    """Count matching prefix bits between two bit arrays."""
...
def encode_context(raw_text, enc):
    """Tokenize raw context text into model token IDs."""
...
def get_model(model_name="gpt2", device=None, seed=1234, model_precision="float32"):
    """Load tokenizer and causal language model with target precision."""
    np.random.seed(seed)
    ...
    if model_precision not in dtype_map:
        print(
            f"Warning: Invalid model_precision '{model_precision}'. Defaulting to float32."
        )
...
def enc32(text):
    """Encode text into the custom 32-symbol bit representation."""
    bits = []
    for c in text:
...
def dec32(bits):
    """Decode custom 32-symbol bits back into text."""
    text = ""
...
def expansion_ratio(message, encoded):
    """Compute expansion ratio between encoded text and source bit length."""
...
SingleExampleOutput = namedtuple(
    "SingleExampleOutput",
    [
        "generated_ids",
...
def get_probs_past(
    model, prev=None, past=None, device="cuda", top_p=1.0, top_k=0, temp=1.0
):
    """
...
def get_lower_upper_bound(cumulative_probs, v):
    """Get lower and upper cumulative bounds for a token interval."""
    lower_bound = cumulative_probs[v - 1] if v > 0 else torch.tensor(0)
    upper_bound = (
        cumulative_probs[v] if v < len(cumulative_probs) - 1 else torch.tensor(1)
    )
...
def func_mrn(k_m, n_m, r):
    """Compute the wrapped sparse-sampling random offset."""
    result = (k_m / n_m) + r
    if result >= 1:
        result = result - 1
    return result
