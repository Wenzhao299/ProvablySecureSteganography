"""Discop steganography implementation using randomized Huffman traversal."""
...
class Node:
    ...
    def __lt__(self, c) -> bool:
        """Return whether this object is less than another object."""
        if isinstance(c, float):
            return self.prob < c
        return self.prob < c.prob

    def __le__(self, c) -> bool:
        """Return whether this object is less than or equal to another object."""
        if isinstance(c, float):
            return self.prob <= c
        return self.prob <= c.prob
...
def create_huffman_tree(
    indices: List[int], probs: List[float], search_for: Optional[int] = None
) -> Tuple[Node, float]:
    """Create huffman tree."""
    sz = len(indices)
    nodes = [
        Node(
            probs[i],
            index=indices[i],
            search_path=(0 if search_for == indices[i] else None),
        )
        for i in range(sz)
    ]
    a = deque(nodes)
    ...
    def fun():
        """Pop the next smallest node from the merge queues."""
        nonlocal a, b
...
def encode_step(probs, indices, message_bits, baseline):
    ...
    while not node.is_leaf():  # 如果不是叶子节点，则往下执行
        ...
        node = (
            path_table["0"] if msg_exhausted_flag else path_table[message_bits[n_bits]]
        )

        if (
            path_table["0"] != path_table["1"]
        ):  # 未出现歧义，可以嵌入一个bit，这个nbits可以代表message中消息的位置，从1开始。
            n_bits += 1
...
def encode_discop(
    ...
    while message_len > 0:
        ...
        probs, indices, past = get_probs_past(
            model, prev, past, device, top_p, top_k, temp
        )
        ...
        if baseline_flag:
            sampled_index, n_bits = encode_step_baseline(
                indices, probs, message_bits, device
            )
        else:
            sampled_index, n_bits = encode_step(
                probs, indices, message_bits, baseline_flag
            )
...
def decode_step(probs, indices, tokenID):
    """Execute one decoding step for the current distribution."""
...
def encode_step_baseline(indices, probs, message_bits, device):
    """Execute one baseline encoding step."""
    sampled_index = 0
    n_bits = 0

    probs_cumsum = torch.tensor(probs).cumsum(dim=0).to(device)
    interval_begin = torch.cat(
        (torch.tensor([0], device=probs_cumsum.device), probs_cumsum[:-1]), dim=0
    )
...
def decode_step_baseline(probs, indices, tokenID, device):
    """Execute one baseline decoding step."""
    probs_cumsum = torch.tensor(probs).cumsum(dim=0).to(device)
    interval_begin = torch.cat(
        (torch.tensor([0], device=probs_cumsum.device), probs_cumsum[:-1]), dim=0
    )
