"""ADG-based steganography utilities for token-level encode and decode."""
...
def find_nearest(anum: float, probs: List[float]) -> int:
    ...
    """Find the nearest index by probability difference."""
    up = len(probs) - 1
    if up == 0:
        return 0
    bottom = 0
...
class ADGNode:
    ...
    def grouping(self, device) -> None:
        """Group candidate tokens according to ADG partition rules."""
        probs = self.probs[:]
        ...
        for i in range(num_groups - 1):
            ...
            self.children.append(
                ADGNode(
                    probs_child_i,
                    indices_child_i,
                    self.multiplier / sum(probs_child_i) * (probs_sum / num_groups),
                )
            )
        # groups.append({'probs': probs, 'indices': indices})  # sorted
        self.children.append(
            ADGNode(
                probs, indices, self.multiplier / sum(probs) * (probs_sum / num_groups)
            )
        )
...
def adg_encode_step(
    ...
    """Run one ADG encoding step on the current token distribution."""
...
def adg_decode_step(tokenID, probs: torch.Tensor, indices: torch.Tensor, device):
    """Run one ADG decoding step for a generated token."""
...
def encode_adg(
    ...
    while message_len > 0:
        ...
        sampled_index, total_code_len = adg_encode_step(
            probs=probs,
            indices=indices,
...
@torch.no_grad()
def decode_adg(
    model, generated_ids, context, device="cuda", top_p=1.0, top_k=0, temp=1.0
):
    """Decode data with adg."""
    context = torch.tensor(context[-1022:], device=device, dtype=torch.long)
    past = None
    prev = context
    extracted_message = []
    ...
    for tokenID in generated_ids:
        ...
        cur_extracted_message = adg_decode_step(
            probs=probs, indices=indices, tokenID=tokenID, device=device
        )
