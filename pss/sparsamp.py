...
def encode_step(probs, n_m, k_m):
    ...
    # temp0 = ceil(SE[0] * n_m - r * n_m)
    # temp1 = ceil((SE[1] * n_m - r * n_m))
...
#     stat_time = 0
#
#     while token_num_generated < token_num_need_generation:
...
#
#         generated_ids.append(tokenID)
#         token_num_generated += 1
#         prev = torch.tensor([tokenID], device=device, dtype=torch.long).unsqueeze(0)
#
#     return generated_ids, encoded_message, total_entropy, stat_time


@torch.no_grad()
def encode_spar(model, context, message_bits, token_num_need_generation, device='cuda', block_size=32, top_p=1.0, top_k=0, random_seed=42, temp=1.0):
    context = torch.tensor(context[-1022:], device=device, dtype=torch.long)
    ...
    while m_index < message_len:
        if token_num_generated >= token_num_need_generation:
            # print(f"Warning: Token limit ({token_num_need_generation}) reached, but message not fully encoded. Encoded {m_index} of {message_len} bits.")
            break
...
@torch.no_grad()
def encode_spar2(model, context, message_bits, token_num_need_generation, device='cuda', block_size=32, top_p=1.0, top_k=0, random_seed=42, temp=1.0):
    context = torch.tensor(context[-1022:], device=device, dtype=torch.long)
...
#
#         SE = get_lower_upper_bound(cumulative_probs, token_index)
#         temp0_ = (SE[0] * n_m - r * n_m)
#         temp0 = ceil(temp0_)
#         temp1_ = (SE[1] * n_m - r * n_m)
#         temp1 = ceil(temp1_)
#         n_m = temp1 - temp0
...
@torch.no_grad()
def decode_spar(model, generated_ids, context, device='cuda', block_size=32, top_p=1.0, top_k=0, random_seed=42, temp=1.0):
    context = torch.tensor(context[-1022:], device=device, dtype=torch.long)
