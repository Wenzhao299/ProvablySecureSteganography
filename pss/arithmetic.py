"""Arithmetic-coding-based steganography encoder and decoder."""
    ...
    with torch.no_grad():
        ...
        while i < message_len:
            ...
            if (probs < cur_threshold).nonzero().numel() > 0:
                k = max(
                    2, (probs < cur_threshold).nonzero()[0].item()
                )  # not less than 2
            ...
            new_int_bottom_bits_inc = list(
                reversed(int2bits(new_int_bottom, precision))
            )
            ...
            cur_interval[1] = (
                bits2int(reversed(new_int_top_bits)) + 1
            )  # +1 here because upper bound is exclusive
